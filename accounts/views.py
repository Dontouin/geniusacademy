...
def register_choice(request):
    """Page to choose type of user to register: student, parent, lecturer"""
...
from django.contrib import messages
from .forms import StudentAddForm, ParentAddForm, TeacherAddForm

def register_user(request, user_type):
    """
    Vue universelle pour enregistrer un utilisateur selon son type.
    user_type: 'student', 'parent', 'lecturer'
    """
    form = None
    template = None
    ...
    if user_type == "student":
        ...
        form = TeacherAddForm(request.POST or None)
...
def profile(request):
    ...
    if request.user.is_student:
        ...
        parent = get_object_or_404(Parent, parent=request.user)  # CORRECTION : 'parent=' au lieu de 'user='
...
def edit_parent(request, pk):
    ...
    if request.method == "POST":
        form = ProfileUpdateForm(request.POST, request.FILES, instance=parent_obj.user)
        if form.is_valid():
            form.save()
            messages.success(request, f"Parent {parent_obj.user.get_full_name} updated.")
            return redirect("parent_list")
        messages.error(request, "Correct the errors below.")
    else:
        form = ProfileUpdateForm(instance=parent_obj.user)
...
def delete_parent(request, pk):
    parent_obj = get_object_or_404(Parent, pk=pk)
    full_name = parent_obj.user.get_full_name
    parent_obj.delete()
...
from django.contrib import messages
from django.shortcuts import render, redirect
from accounts.models import Teacher, TeacherInfo
from accounts.forms import TeacherInfoForm
...
def teacher_info_add(request):
    ...
    teacher, created = Teacher.objects.get_or_create(
        user=user,
        defaults={'speciality': '', 'diploma': ''}  # valeurs par défaut
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            info.teacher = teacher  # on utilise l'objet existant ou créé
            info.save()
            messages.success(request, "Teacher information sheet saved.")
            return redirect("teacher_info_pdf", pk=info.pk)  # Redirection vers PDF
        messages.error(request, "Correct the errors below.")
    else:
...
def teacher_info_pdf(request, pk):
    ...
    try:
        info = TeacherInfo.objects.get(pk=pk)
    ...
    logo_size = 100  # Taille agrandie
    logo_y_offset = 135 - (logo_size - 60)/2  # Ajustement vertical pour centrer
    ...
    lignes = [
        ...
        ("Numero d'urgence",f" {info.contact_urgence or ''}")
    ]
    ...
    # === Infos Professionnelles remontées ===
    y_top2 = y - 20
    y_title2 = y_top2 - 10
    p.setFont("Helvetica-Bold", 12)
    ...
    p.drawString(right_x, y_sign_admin - 20, "Directeur Générale")
...
from django.contrib import messages
from django.contrib.auth.decorators import login_required
# from .models import Reservation, Course, User
from accounts.utils import send_sms  # fonction SMS
...
from accounts.filters import LecturerFilter
